...
월 정산에 배달의민족·쿠팡이츠·요기요·땡겨요별 정산 대상 원본금액, POS 중복 제외액, 손익 추가액과 원본 등록 여부를 표시한다. 원본금액은 부가세 포함·환불 차감액이며, 손익 추가액은 저장한 부가세와 채널별 POS 포함 기준을 따른다. 원본 미등록은 0원과 구별한다. 거래 제외·금액 및 귀속월 오류·중복 거래·수수료·지급예정액은 이 표의 매출에 넣지 않는다. 미확인 세액·분류는 기존 손익과 같은 잠정 결과로 표시하며 확정 차단을 유지한다.

화면과 XLSX는 서버 계산의 `platformRevenueByChannel`을 사용한다. 확정 정산은 당시 보관본을 사용하고, 새 필드가 없던 보관본에는 채널별 금액을 새로 만들어 넣지 않는다. 기존 손익·계약상 배분 규칙이나 원본 데이터는 변경하지 않는다. DB 마이그레이션은 없으며 같은 배포 브랜치에서 ODA API 다음 ODA 웹을 배포한다.

검증: `npm run test:oda` 284개 통과, 선택적 PostgreSQL 통합검사 1개 건너뜀. `npm run build:oda` 통과. 혼합 POS 포함 기준, 환불, 실제 VAT, 제외·중복·오류 행, 원본 미등록과 0원의 구분, 기준·원본 화면 이동, 과거 확정본 보존을 검사했다.
