...
function fixture(additionalTerms = "매장 운영에 필요한 업무를 성실하게 수행합니다.") {
  let sequence = 0;
  ...
  const draft = createNativeContract({ storeId: "store-oda", employeeId: "staff-1", employeeActorId: "staff-login-1", employeeName: "김도윤", title: "ODA 외대점 근로계약서", templateKey: "oda-labor-v1", terms: {
  ...
  const completed = signNativeContract(signed, { expectedVersion: signed.version, role: "employee", typedName: "김도윤", consent: true, consentVersion: NATIVE_ESIGN_CONSENT_VERSION, documentHash: signed.documentHash,
