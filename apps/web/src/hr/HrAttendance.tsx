...
type AttendanceProps = HrPanelProps & { tab: 'attendance' | 'leave' | 'shifts'; hideClock?: boolean; entryIntent?: StaffEntryIntent };
type Employee = { id: string; name: string };
type Command = (type: string, input: Record<string, unknown>, message?: string) => Promise<boolean>;
type Panel = { state: HrAttendanceState; settings: HrPanelProps['workspace']['settings']; employees: Employee[]; employeeId?: string; manage: boolean; busy: boolean; run: Command; filterEmployee: string; month: string; error: string; hideClock?: boolean; onCreate?: () => void; onOpenRecord?: (id: string) => void };
...
export function HrAttendance({ workspace, permissions, mutate, busy, employeeId, actorId, tab, hideClock, entryIntent }: AttendanceProps) {
  ...
  const lock = useRef(false);
  const [entryDialog, setEntryDialog] = useState<'create'|'detail'|'edit'|null>(null);
  const [entryId, setEntryId] = useState('');
  const [entrySnapshot, setEntrySnapshot] = useState<HrWorkEntry|null>(null);
  const canWrite = permissions.manage || Boolean(permissions.self && employeeId);
  const work = tab==='attendance' ? workspace.attendance.workEntries.find(row=>row.id===entryId && (permissions.manage||row.employeeId===employeeId)) : undefined;
  const leave = tab==='leave' ? workspace.attendance.leaveRequests.find(row=>row.id===entryId && (permissions.manage||row.employeeId===employeeId)) : undefined;
  const record = work || leave;
  const recordLocked = work ? isHrAttendanceLocked(workspace.attendance,work.employeeId,work.date)||isHrAttendanceLocked(workspace.attendance,work.employeeId,work.endDate) : leave ? leave.slots.some(slot=>isHrAttendanceLocked(workspace.attendance,leave.employeeId,slot.date)) : false;
  useStaffEntryIntent(JSON.stringify([workspace.storeId,actorId,employeeId,tab]),entryIntent,intent=>{
    setEntryDialog(null);setEntryId('');setEntrySnapshot(null);setError('');setMessage('');
    if(intent?.action==='create'){if(canWrite && tab!=='shifts')setEntryDialog('create');return;}
    if(!intent?.recordId)return;
    const target = tab==='attendance' ? workspace.attendance.workEntries.find(row=>row.id===intent.recordId && (permissions.manage||row.employeeId===employeeId)) : tab==='leave' ? workspace.attendance.leaveRequests.find(row=>row.id===intent.recordId && (permissions.manage||row.employeeId===employeeId)) : undefined;
    if(!target){setError(unavailableStaffRecord);return;}
    setMonth(('date' in target ? target.date : target.startDate).slice(0,7));setFilterEmployee('');setEntryId(target.id);setEntryDialog('detail');
  });
  ...
  const props: Panel = { state: workspace.attendance, settings: workspace.settings, employees, employeeId, manage: permissions.manage, busy: busy || saving, run, filterEmployee: permissions.manage ? filterEmployee : employeeId ?? '__unlinked__', month, error,
    ...(canWrite && tab !== 'shifts' ? { onCreate: () => { if (busy || saving) return; setError(''); setMessage(''); setEntryId(''); setEntrySnapshot(null); setEntryDialog('create'); } } : {}),
    onOpenRecord: id => {
      if (busy || saving) return;
      const target = tab === 'attendance' ? workspace.attendance.workEntries.find(row => row.id === id && (permissions.manage || row.employeeId === employeeId)) : tab === 'leave' ? workspace.attendance.leaveRequests.find(row => row.id === id && (permissions.manage || row.employeeId === employeeId)) : undefined;
      setError(''); setMessage(''); setEntrySnapshot(null);
      if (!target) { setError(unavailableStaffRecord); setEntryDialog(null); return; }
      setEntryId(target.id); setEntryDialog('detail');
    },
  };
  return <div className="hr-stack" aria-label={tab === 'attendance' ? '근무 관리' : tab === 'leave' ? '휴가 관리' : '교대근무표'}>
    ...
    {entryDialog==='create' && canWrite && tab!=='shifts' && <HrDialog title={tab==='leave'?'휴가 신청':'근무 등록·정정 신청'} busy={busy||saving} onClose={()=>setEntryDialog(null)}>
      {error&&<p className="hr-error" role="alert">{error}</p>}
      {tab==='leave' ? <LeaveRequestForm {...props} onSaved={()=>setEntryDialog(null)} /> : <Form busy={busy||saving} submit="근무 기록 저장" onSave={async data=>{const saved=await run('work.create',workInput(data));if(saved)setEntryDialog(null);return saved;}}><WorkFields employees={employees} employeeId={employeeId} manage={permissions.manage} /></Form>}
    </HrDialog>}
    {entryDialog==='detail' && record && <HrDialog title={work?'근무 신청 상세':'휴가 신청 상세'} busy={busy||saving} onClose={()=>setEntryDialog(null)}>
      <p>{employees.find(row=>row.id===record.employeeId)?.name??'구성원'} · <State state={record.status} />{recordLocked&&<span className="hr-badge">마감</span>}</p>
      {work&&<><p>{work.date} {work.startTime} ~ {work.endDate} {work.endTime}</p><p>휴게 {work.breakMinutes}분 · 인정 시간 {duration(work.recognizedMinutes)}</p></>}
      {leave&&<><p>{workspace.attendance.leaveTypes.find(row=>row.id===leave.typeId)?.name??'휴가'} · {leave.startDate} ~ {leave.endDate}</p><p>{leave.paid?'유급':'무급'} · 총 {duration(leave.minutes)}</p><ul>{leave.slots.map(slot=><li key={slot.date}>{slot.date} {slot.startTime} ~ {slot.endTime} · {duration(slot.minutes)}</li>)}</ul></>}
      <p className="hr-prewrap">{record.note || '작성한 사유가 없습니다.'}</p>
      {error&&<p className="hr-error" role="alert">{error}</p>}
      {canWrite&&!recordLocked&&['pending','approved'].includes(record.status)&&<div className="hr-actions">{work&&<Button variant="secondary" disabled={busy||saving} onClick={()=>{setEntrySnapshot(work);setEntryDialog('edit');}}>근무 기록 수정</Button>}<Button variant="secondary" disabled={busy||saving} onClick={()=>void run(work?'work.cancel':'leave.cancel',{id:record.id,expectedRevision:record.revision},'신청을 취소했습니다.')}>신청 취소</Button></div>}
    </HrDialog>}
    {entryDialog==='edit' && work && entrySnapshot?.id===work.id && canWrite && !recordLocked && ['pending','approved'].includes(work.status) && <HrDialog title="근무 기록 수정" busy={busy||saving} onClose={()=>setEntryDialog(null)}>
      {error&&<p className="hr-error" role="alert">{error}</p>}<Form busy={busy||saving} submit="수정 저장" onSave={async data=>{const saved=await run('work.update',{id:entrySnapshot.id,expectedRevision:entrySnapshot.revision,...workInput(data)});if(saved)setEntryDialog(null);return saved;}}><WorkFields employees={employees} employeeId={employeeId} manage={permissions.manage} entry={entrySnapshot}/></Form>
    </HrDialog>}
...
function WorkPanel(props: Panel) {
  ...
  return <>
    ...
    {manage && <Card title="근무 기록 추가" description="실제 근무 시간과 휴게 시간을 입력하세요. 밤을 넘겨 근무했다면 종료일을 다음 날로 지정하세요."><Form busy={busy} submit="근무 기록 저장" onSave={data => run('work.create', workInput(data))}><WorkFields employees={employees} employeeId={employeeId} manage={manage} /></Form></Card>}
    {!manage && editable && <div className="staff-panel-action"><p className="hr-muted">빠진 근무를 등록하거나 기록을 정정하면 관리자 승인 후 반영됩니다.</p><Button disabled={busy || !props.onCreate} onClick={props.onCreate}>근무 정정 신청</Button></div>}
...
function LeaveRequestForm({ state, employees, employeeId, manage, busy, run, onSaved }: Panel & { onSaved?: () => void }) {
...
function LeavePanel(props: Panel) {
  ...
  return <>
    <Card title="휴가 잔액" description="사용 가능한 잔액에서 승인 대기 중인 신청분은 이미 예약됩니다. 취소·반려하면 원래 지급분의 유효기간으로 복원됩니다."><div className="hr-toolbar"><Field title="잔액 기준일"><input type="date" required value={balanceDate} onChange={event => setBalanceDate(event.target.value || hrToday())} /></Field></div>
      {balanceEmployees.length && state.leaveTypes.some(row => row.deductBalance) ? !manage ? <div className="staff-leave-balances">{balanceEmployees.flatMap(employee => state.leaveTypes.filter(type => type.deductBalance).map(type => { const balance = getHrLeaveBalance(state, employee.id, type.id, balanceDate); return <div className="staff-leave-balance" key={`${employee.id}-${type.id}`}><span>{type.name}</span><strong>{duration(balance.availableMinutes)}</strong><span className="staff-record-meta">사용 가능 · 승인 대기 예약 {duration(balance.reservedMinutes)}</span></div>; }))}</div> : <div className="hr-table-wrap"><table className="hr-table"><thead><tr><th>구성원</th><th>휴가 종류</th><th>사용 가능</th><th>승인 대기 예약</th></tr></thead><tbody>{balanceEmployees.flatMap(employee => state.leaveTypes.filter(type => type.deductBalance).map(type => { const balance = getHrLeaveBalance(state, employee.id, type.id, balanceDate); return <tr key={`${employee.id}-${type.id}`}><td>{employee.name}</td><td>{type.name}</td><td>{duration(balance.availableMinutes)}</td><td>{duration(balance.reservedMinutes)}</td></tr>; }))}</tbody></table></div> : <Empty>잔액을 조회할 구성원이나 잔액 차감형 휴가가 없습니다.</Empty>}
    </Card>
    {manage && <Card title="휴가 신청" description="근무일별 사용 시간과 사유를 입력하세요. 적용 근무유형과 게시된 교대가 없으면 월~금 09:00~18:00, 하루 최대 480분을 기준으로 휴가를 검증합니다. 시급 급여는 승인된 인정 근무 분과 유급휴가 분으로 계산하며 회사의 하루 기준 시간으로 자동 대체하지 않습니다.">
      <LeaveRequestForm state={state} settings={settings} employees={employees} employeeId={employeeId} manage={manage} busy={busy} run={run} filterEmployee={filterEmployee} month={month} error="" />
    </Card>}
    {!manage && employeeId && <div className="staff-panel-action"><p className="hr-muted">휴가 종류와 기간을 선택해 신청하세요. 처리 상태는 아래에서 확인할 수 있어요.</p><Button disabled={busy || !props.onCreate} onClick={props.onCreate}>휴가 신청</Button></div>}
    ...
    <Card title="휴가 지급·차감 원장" description="지급, 사용 예약, 복원 내역을 발생 순서대로 보관합니다.">{ledger.length ? !manage ? <ul className="staff-leave-ledger">{ledger.map(row => <li key={row.id}><span className="staff-record-head"><strong>{state.leaveTypes.find(type => type.id === row.typeId)?.name ?? "휴가"} · {{ grant: "지급", use: "사용·예약", restore: "복원" }[row.kind]}</strong><span>{row.minutes > 0 ? "+" : ""}{row.minutes.toLocaleString("ko-KR")}분</span></span><span className="staff-record-meta">유효기간 {row.effectiveFrom} ~ {row.expiresOn}</span>{row.note && <p className="hr-prewrap">{row.note}</p>}</li>)}</ul> : <div className="hr-table-wrap"><table className="hr-table"><thead><tr><th>구성원·휴가</th><th>내역</th><th>증감</th><th>유효기간</th><th>메모</th></tr></thead><tbody>{ledger.map(row => <tr key={row.id}><td>{employeeName(row.employeeId)} · {state.leaveTypes.find(type => type.id === row.typeId)?.name ?? row.typeId}</td><td>{{ grant: '지급', use: '사용·예약', restore: '복원' }[row.kind]}</td><td>{row.minutes > 0 ? '+' : ''}{row.minutes.toLocaleString('ko-KR')}분</td><td>{row.effectiveFrom}~{row.expiresOn}</td><td>{row.note || '—'}</td></tr>)}</tbody></table></div> : <Empty>조회 조건에 맞는 지급·차감 내역이 없습니다.</Empty>}</Card>
  </>;
}
