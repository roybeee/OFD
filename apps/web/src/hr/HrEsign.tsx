...
function NativeContracts(props: Props) {
  ...
  function navigate(next: View) { if (locked.current) return; detailRequest.current++; setView(next); setLoading(false); setDetail(null); setCopySource(null); setError(''); setSuccess(''); }
  async function run(suffix: string, input: Record<string, unknown>, message: string): Promise<boolean> {
    ...
    try {
      ...
      setData(result); if (result.contract) { setCopySource(null); setDetail(result.contract); setView('detail'); }
  ...
  return <>
    ...
    {(view === 'create' || view === 'edit') && manage && validData && <ContractForm sourceContract={copySource} contract={view === 'edit' ? detail : null} employers={validData.employers.filter(row => row.active)} employees={props.workspace.employees.filter(row => row.status !== 'retired')} busy={disabled} onSubmit={input => run('/contracts', input, '계약 초안을 저장했습니다. 전체 내용을 확인한 뒤 서명을 요청해 주세요.')} />}
    {view === 'detail' && detail && <ContractDetail key={`${detail.id}:${detail.version}`} contract={detail} actorId={props.actorId} manage={manage} busy={disabled} onEdit={() => setView('edit')} onCopy={() => { setCopySource(detail); setView('create'); setDetail(null); setSuccess('기존 조건을 불러왔습니다. 새 계약 기간과 조건을 검토한 뒤 초안을 저장해 주세요.'); }} onAction={(action, input, message) => run(`/contracts/${encodeURIComponent(detail.id)}/${action}`, { expectedVersion: detail.version, ...input }, message)} onDownload={download} />}
  </>;
}

type Action = (action: string, input: Record<string, unknown>, message: string) => Promise<boolean>;
function ContractDetail({ contract: c, actorId, manage, busy, onEdit, onCopy, onAction, onDownload }: { contract: NativeContract; actorId: string; manage: boolean; busy: boolean; onEdit: () => void; onCopy: () => void; onAction: Action; onDownload: (kind: 'pdf' | 'evidence') => Promise<void> }) {
...
function ContractForm({ employers, employees, busy, onSubmit, contract, sourceContract }: { contract?: NativeContract | null; sourceContract?: NativeContract | null; employers: NativeEmployer[]; employees: Props['workspace']['employees']; busy: boolean; onSubmit: (input: Record<string, unknown>) => Promise<boolean> }) {
  const initial = contract ?? sourceContract;
