...
export function EsignBatchForm({ template, employer, workspace, accounts, busy, onSubmit }: {
  ...
  // Freeze the reviewed personnel snapshot; a changed workspace requires reviewing again.
  ...
  const reviewing = reviewVersion !== null;
  ...
  return <form className="hr-form esign-form esign-batch-form" onSubmit={event => {
    ...
    if (!reviewing || reviewVersion !== workspace.version) { setReviewVersion(workspace.version); setConfirmed(false); return; }
    if (!confirmed || !employer) return;
    void onSubmit({ expectedVersion: 0, savedTemplateId: template.id, savedTemplateVersion: template.version, expectedEmployerVersion: employer.version,
      expectedHrVersion: reviewVersion, title: title.trim(), effectiveDate: start, endDate: end, employeeIds: selected });
    ...
    {reviewing && <><h3>{title} · {recipients.length}명</h3><p>적용 기간 {start} ~ {end || '종료일 없음'}</p><div className="esign-batch-review">{recipients.map(employee => <div className="esign-delivery" key={employee.id}><strong>{employee.name} · {employee.employeeNumber}</strong><p>현재 인사정보: {employee.payType === 'monthly' ? '월급' : '시급'} {employee.basePay.toLocaleString('ko-KR')}원</p><p>새 계약 초안: {terms.payType === 'monthly' ? '월급' : '시급'} {terms.basePay.toLocaleString('ko-KR')}원</p></div>)}</div>
      {reviewVersion !== workspace.version && <p role="alert">직원 정보가 갱신되었습니다. 대상자 선택으로 돌아가 다시 확인해 주세요.</p>}
      <label className="esign-consent"><input type="checkbox" checked={confirmed} disabled={busy || reviewVersion !== workspace.version} onChange={event => setConfirmed(event.target.checked)} /><span>대상자별 임금과 공통 조건을 확인했으며, 추가 약정에 다른 직원의 개인정보가 없는지 검토했습니다.</span></label></>}
    <div className="esign-actions">{reviewing && <Button type="button" variant="secondary" disabled={busy} onClick={() => { setReviewVersion(null); setConfirmed(false); }}>대상자 선택으로</Button>}<Button type="submit" disabled={busy || !valid || (reviewing && (!confirmed || reviewVersion !== workspace.version))}>{busy ? '저장 중…' : reviewing ? `${recipients.length}명 초안 저장` : '선택한 직원과 조건 검토'}</Button></div>
  </form>;
}
