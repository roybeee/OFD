...
function People(props: Props) {
  ...
  return <>
    <section className="hr-card hr-people-card"><div className="hr-section-heading"><div><h2>직원·조직</h2><p>직원 정보를 찾고 소속 조직과 변경 이력을 확인하세요.</p></div><div className="hr-actions"><Button variant="secondary" onClick={exportDirectory} disabled={!filtered.length}><ArrowDownToLine size={15} /> 목록 내려받기</Button>{permissions.manage && <Button disabled={busy} onClick={() => setEditor('new')}><Plus size={16} /> 직원 등록</Button>}</div></div>
      <div className="hr-actions hr-view-tabs" aria-label="직원 보기 방식"><Button variant={view === 'directory' ? 'primary' : 'secondary'} aria-pressed={view === 'directory'} onClick={() => setView('directory')}>직원 목록</Button><Button variant={view === 'organization' ? 'primary' : 'secondary'} aria-pressed={view === 'organization'} onClick={() => setView('organization')}>조직도</Button></div>
