...
export function DriverTodayPage({ data, source, notify, refresh }: { data: BootstrapData; source: 'live' | 'demo'; notify: (message: string, tone?: 'success' | 'info' | 'warning') => void; refresh: () => void }) {
  const [selected, setSelected] = useState<Delivery | null>(null);
  const [completed, setCompleted] = useState<string[]>([]);
  const nextDelivery = data.deliveries.find((delivery) => delivery.status === 'driving');
  const completedCount = data.deliveries.filter((delivery) => delivery.status === 'delivered' || completed.includes(delivery.id)).length;
  ...
  async function completeDelivery(delivery: Delivery, file: File | null, recipientName: string) {
    if (delivery.status !== 'driving' || completed.includes(delivery.id)) throw new Error('운행 중인 배송만 완료할 수 있습니다.');
    if (source === 'demo') {
      setCompleted((current) => current.includes(delivery.id) ? current : [...current, delivery.id]);
      setSelected(null);
      notify(`${delivery.storeName} 배송 완료를 데모 화면에 반영했습니다.`, 'success');
      return;
    }
    ...
    try {
      ...
      const upload = await mutateV2<{ objectKey: string; uploadUrl: string; requiredHeaders?: Record<string, string> }>(`/shipments/${delivery.id}/proof-upload`, { contentType }, { idempotencyKey: newIdempotencyKey(), actorId });
      ...
      await mutateV2(`/shipments/${delivery.id}/deliver`, { expectedVersion: delivery.version ?? 1, photoKey: upload.objectKey, recipientName, capturedAt: new Date().toISOString() }, { idempotencyKey: newIdempotencyKey(), actorId });
      setSelected(null);
  ...
  return (
    <main id="main-content" data-testid="driver-today-screen" className="page page-driver" tabIndex={-1}>
      ...
      {selected && <DeliveryDrawer delivery={selected} completed={completed.includes(selected.id) || selected.status === 'delivered'} actionable={selected.status === 'driving' && data.capabilities.includes('driver.deliveries.complete')} onClose={() => setSelected(null)} onComplete={(file, recipientName) => completeDelivery(selected, file, recipientName)} />}
    </main>
  );
}
  ...
  return (
    <div className="drawer-backdrop" role="presentation">
      <aside ref={dialogRef} tabIndex={-1} className="delivery-drawer" role="dialog" aria-modal="true" aria-labelledby="delivery-title">
        <header><div><span className="drawer-kicker">{completed ? '완료된 배송' : actionable ? `${delivery.sequence}번째 배송` : '출발 전 배송'}</span><h2 id="delivery-title">{delivery.storeName}</h2></div><button type="button" className="icon-button" data-dialog-initial aria-label="배송 상세 닫기" onClick={onClose}><X size={22} /></button></header>
