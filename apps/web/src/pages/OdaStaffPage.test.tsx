...
async function click(label: string, within: ParentNode = container) {
  const button = [...within.querySelectorAll('button')].find(row => {
...
describe('employee workday home', () => {
  ...
  it('gets a new location even after a preview, uses the refreshed version and prevents double clicks', async () => {
    ...
    await render(); await click('위치 다시 확인'); expect(api.position).toHaveBeenCalledTimes(1);
    const button = [...container.querySelectorAll('button')].find(row => row.textContent?.startsWith('출근하기'))!;
    ...
    await act(async () => resolve(saved)); expect(container.textContent).toContain('출근을 기록했습니다'); expect(container.textContent).toContain('퇴근하기');
  });
  ...
  it('uses the last clock state across midnight and records clock-out with a fresh location', async () => {
    ...
    api.command.mockResolvedValue(saved); await render(); expect(container.textContent).toContain('이전 날짜의 출근 기록');
    await click('퇴근하기'); expect(api.command).toHaveBeenCalledWith('s1', 0, 'clock.out', { employeeId: 'e1', location: position() });
    expect(container.textContent).toContain('퇴근 완료');
  });
  ...
  it('stops if another device changed the clock state while refreshing', async () => {
    ...
    expect(container.textContent).toContain('출퇴근 상태가 변경'); expect(container.textContent).toContain('퇴근하기');
  });
  ...
  it('shows only published schedules, separates own and team days, and orders pinned published notices first', async () => {
    ...
    api.get.mockResolvedValue(r); await render(); expect(container.textContent).not.toContain('팀 동료'); expect(container.textContent).toContain('(다음 날)');
    expect(container.querySelector('[aria-label="2026-09-16 일정 없음"]')).toBeTruthy();
    await click('매장'); expect(container.textContent).toContain('팀 동료'); expect(container.querySelector('[aria-label="2026-09-15 2명"]')).toBeTruthy();
  ...
  it('blocks unlinked employees and unset locations without leaking the team schedule', async () => {
    ...
    expect(container.textContent).toContain('관리자에게 직원 계정 연결'); expect(container.textContent).toContain('위치가 아직 등록되지 않았습니다');
    expect([...container.querySelectorAll('button')].find(row => row.textContent?.startsWith('출근하기'))!.disabled).toBe(true);
    expect([...container.querySelectorAll('button')].find(row => row.textContent === '매장')!.disabled).toBe(true);
  ...
  it('updates the Korean date across midnight and refreshes on focus', async () => {
    ...
    expect(container.textContent).toContain('2026년 9월 16일'); expect(api.get).toHaveBeenCalledTimes(2);
  ...
  it('routes staff into the focused home and retains only personal menus without legacy clock buttons', async () => {
    ...
    await click('근무 기록·정정 신청', container.querySelector('[aria-label="내 인사 메뉴"]')!);
    ...
    await click('직원 홈으로 돌아가기'); expect(container.textContent).toContain('출근하기');
  });
});
