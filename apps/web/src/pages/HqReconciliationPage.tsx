...
export function HqReconciliationPage({ data, notify, refresh }: { data: BootstrapData; notify: (message: string, tone?: 'success' | 'info' | 'warning') => void; refresh: () => void }) {
  ...
  const [reverseReasonByPayment, setReverseReasonByPayment] = useState<Record<string, string>>({});
  ...
  async function match(item: BankMatch) {
    ...
    try {
      await mutateV2(`/payments/${paymentRequestId}/manual-match`, { expectedVersion: version, bankTransactionId }, {
  ...
  async function requestSync() {
    ...
    try {
      ...
      notify('계좌 거래 수집 요청을 접수했습니다.', 'success');
      refresh();
    } catch (error) { notify(error instanceof Error ? error.message : '계좌 수집 요청에 실패했습니다.', 'warning'); }
    finally { setSyncing(false); }
  }
  async function reverseMatch(item: BankMatch) {
    const paymentRequestId = item.paymentRequestId;
    const reason = paymentRequestId ? reverseReasonByPayment[paymentRequestId]?.trim() : '';
    if (!paymentRequestId || !item.bankTransactionId) { notify('연결된 청구 식별정보가 없어 취소할 수 없습니다.', 'warning'); return; }
    if (!reason || reason.length < 3) { notify('대사 취소 사유를 3자 이상 입력해 주세요.', 'warning'); return; }
    setReversingId(paymentRequestId);
    try {
      await mutateV2(`/payments/${paymentRequestId}/reverse-match`, { expectedVersion: item.version ?? 1, reason }, {
        idempotencyKey: newIdempotencyKey(),
      });
      notify(`${formatMoney(item.amount)} 입금 연결을 취소했습니다. 거래는 다시 검토 큐로 이동합니다.`, 'warning');
      refresh();
  ...
  return (
    <main id="main-content" data-testid="hq-reconciliation-screen" className="page page-hq" tabIndex={-1}>
      ...
      <section className="panel reconcile-panel"><div className="queue-toolbar"><div><h2>입금 처리 큐</h2><p>입금자명과 금액을 청구 내역에 연결합니다.</p></div><div className="filter-chips" role="group" aria-label="입금 대사 필터"><button className={tab === 'manual_review' ? 'active danger' : ''} aria-pressed={tab === 'manual_review'} onClick={() => setTab('manual_review')}>검토 필요 <span>{reviewCount}</span></button><button className={tab === 'auto_matched' ? 'active' : ''} aria-pressed={tab === 'auto_matched'} onClick={() => setTab('auto_matched')}>자동 일치 <span>{matchedCount}</span></button><button className={tab === 'overdue' ? 'active' : ''} aria-pressed={tab === 'overdue'} onClick={() => setTab('overdue')}>연체 <span>{overdueCount}</span></button></div></div>
        <div className="match-list">{selected.map((item) => { const transactionId = item.bankTransactionId ?? item.id; const chosen = candidateByTransaction[transactionId]; const canMatch = Boolean(item.paymentRequestId || chosen); const reverseReason = item.paymentRequestId ? reverseReasonByPayment[item.paymentRequestId] ?? '' : ''; return <article className="match-card" key={item.id}><div className="bank-entry"><span className="bank-icon"><Landmark size={20} /></span><div><small>입금 거래 · {item.transferredAt}</small><strong>{item.depositor}</strong><span>{formatMoney(item.amount)}</span></div></div><div className="match-connector"><span /><Link2 size={18} /><span /></div><div className="invoice-candidate"><small>{item.status === 'manual_review' ? `일치 후보 ${item.candidates ?? 0}건` : item.status === 'overdue' ? '연체 청구' : '연결된 청구'}</small>{item.status === 'manual_review' && item.candidateOptions?.length ? <label className="compact-select"><span className="sr-only">{item.depositor} 입금에 연결할 청구</span><select value={chosen ?? ''} onChange={(event) => setCandidateByTransaction((current) => ({ ...current, [transactionId]: event.target.value }))}><option value="">청구를 선택하세요</option>{item.candidateOptions.map((candidate) => <option key={candidate.paymentRequestId} value={candidate.paymentRequestId}>{candidate.label}</option>)}</select></label> : <strong>{item.storeName ?? '일치 후보가 없습니다'}</strong>}<span>{item.status === 'manual_review' ? '금액과 매장을 직접 확인한 뒤 연결하세요' : '결제 요청 연결 완료'}</span>{item.status === 'auto_matched' && item.paymentRequestId && <label className="reverse-reason"><span>연결 취소 사유</span><input value={reverseReason} maxLength={500} onChange={(event) => setReverseReasonByPayment((current) => ({ ...current, [item.paymentRequestId!]: event.target.value }))} placeholder="예: 다른 매장 입금" /></label>}</div><div className="match-action"><StatusBadge status={item.status} />{item.status === 'manual_review' && <Button disabled={!canMatch} onClick={() => match(item)}><Search size={17} /> {canMatch ? '선택 청구에 연결' : '청구 선택 필요'}</Button>}{item.status === 'auto_matched' && <><span className="matched-check"><Check size={18} /> 처리 완료</span><Button variant="secondary" disabled={!item.paymentRequestId || !item.bankTransactionId || reverseReason.trim().length < 3 || reversingId === item.paymentRequestId} onClick={() => reverseMatch(item)}>{reversingId === item.paymentRequestId ? '취소 중…' : '연결 취소'}</Button></>}</div></article>; })}</div>
