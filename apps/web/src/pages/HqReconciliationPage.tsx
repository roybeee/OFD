...
export function HqReconciliationPage({ data, source, notify, refresh }: { data: BootstrapData; source: 'live' | 'demo'; notify: (message: string, tone?: 'success' | 'info' | 'warning') => void; refresh: () => void }) {
  ...
  async function reverseMatch(item: BankMatch) {
    ...
    try {
      if (source === 'live') await mutateV2(`/payments/${paymentRequestId}/reverse-match`, { expectedVersion: item.version ?? 1, reason }, {
        idempotencyKey: newIdempotencyKey(), actorId: data.meta.appMode === 'demo' ? data.actor.id : undefined,
      });
      else setMatches((current) => current.map((entry) => entry.id === item.id ? { ...entry, status: 'manual_review', paymentRequestId: undefined, storeName: undefined } : entry));
