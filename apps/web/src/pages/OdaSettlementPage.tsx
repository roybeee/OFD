...
export function OdaSettlementPage({ data, notify }: Props) {
  ...
  const upload = state && <UploadBox key={`${storeId}/${month}`} storeId={storeId} month={month} disabled={!editable || Boolean(busy)} busy={busy === 'import'} onImport={importFiles} onReading={setReadingUpload} notify={notify} />;

  return <main id="main-content" className="page oda-page" tabIndex={-1} data-testid="oda-settlement-screen">
    <header className="oda-heading"><div><p className="oda-kicker">ODA PIZZERIA · OPERATIONS</p><h1>월 정산</h1><p>자료를 모으면, 확인할 항목만 남습니다.</p></div><div className="oda-context"><label className="oda-field">매장<select aria-label="정산 매장" value={storeId} disabled={Boolean(busy) || readingUpload} onChange={(event) => setStoreId(event.target.value)}>{stores.map((store) => <option key={store.id} value={store.id}>{store.name}</option>)}</select></label><label className="oda-field">정산월<input type="month" aria-label="정산월" value={month} disabled={Boolean(busy) || readingUpload} onChange={(event) => { if (event.target.value) setMonth(event.target.value); }} /></label><Button variant="secondary" aria-label="정산 새로고침" disabled={loading || Boolean(busy)} onClick={() => void load()}><RefreshCcw size={18} /></Button></div></header>
    ...
    {loading && !state ? <div className="oda-loading" role="status"><RefreshCcw size={27} /><span>이번 달 정산 자료를 불러옵니다.</span></div> : !state ? <div className="oda-empty"><ReceiptText /><strong>{storeId ? '정산 자료에 연결할 수 없습니다.' : '배정된 매장이 없습니다.'}</strong><p>{storeId ? '연결 상태를 확인하고 다시 불러와 주세요.' : '계정 관리자에게 매장 배정을 요청해 주세요.'}</p></div> : <>
      ...
      <section className="oda-metrics" aria-label="이번 달 정산 요약">
      ...
      {editable && <div className="oda-upload-panel" hidden={!((tab === 'overview' && (!hasLines || showUpload)) || tab === 'transactions')}>{upload}</div>}
      {tab === 'overview' && <>
...
function UploadBox({ storeId, month, disabled, busy, onImport, onReading, notify }: { storeId: string; month: string; disabled: boolean; busy: boolean; onImport: (files: PendingFile[], onSaved?: (file: PendingFile) => void) => Promise<boolean>; onReading: (reading: boolean) => void; notify: Props['notify'] }) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [kind, setKind] = useState<OdaSourceKind>('pos');
...
function LineRow({ line, sources, issues, disabled, storeId, month, onSave, onBankExpense }: { line: OdaLine; sources: OdaSource[]; issues: string[]; disabled: boolean; storeId: string; month: string; onSave: (changes: Record<string, unknown>) => Promise<boolean>; onBankExpense?: (body: Record<string, unknown>) => Promise<boolean> }) {
  ...
  const repeatNeedsSource = line.externalId.startsWith('repeat:') && kind === 'expense' && !sources.some(item => item.id === sourceId);
