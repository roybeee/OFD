...
type Tab = 'overview' | 'expenses' | 'transactions' | 'policy' | 'history' | 'automation';
...
export function odaSettlementLocation(search: string, stores: ReadonlyArray<{ id: string }>) {
  ...
  const tab: Tab = value === 'expenses' || value === 'transactions' || value === 'policy' || value === 'history' || value === 'automation' ? value : 'overview';
...
export function OdaSettlementPage({ data, notify }: Props) {
  ...
  return <main id="main-content" className="page oda-page" tabIndex={-1} data-testid="oda-settlement-screen">
    ...
    {loading && !state ? <div className="oda-loading" role="status"><RefreshCcw size={27} /><span>이번 달 정산 자료를 불러옵니다.</span></div> : !state ? <div className="oda-empty"><ReceiptText /><strong>{storeId ? '정산 자료에 연결할 수 없습니다.' : '배정된 매장이 없습니다.'}</strong><p>{storeId ? '연결 상태를 확인하고 다시 불러와 주세요.' : '계정 관리자에게 매장 배정을 요청해 주세요.'}</p></div> : <>
      ...
      <nav className="oda-tabs" aria-label="정산 상세"><button className={tab === 'automation' ? 'active' : ''} aria-current={tab === 'automation' ? 'page' : undefined} onClick={() => setTab('automation')}>자동화 연결</button><button className={tab === 'overview' ? 'active' : ''} aria-current={tab === 'overview' ? 'page' : undefined} onClick={() => setTab('overview')}>월 정산</button><button className={tab === 'expenses' ? 'active' : ''} aria-current={tab === 'expenses' ? 'page' : undefined} onClick={() => setTab('expenses')}>비용 관리</button><button className={tab === 'transactions' ? 'active' : ''} aria-current={tab === 'transactions' ? 'page' : undefined} onClick={() => setTab('transactions')}>거래·증빙{reviewLines.length > 0 && <span className="oda-tab-count">{reviewLines.length}</span>}</button><button className={tab === 'policy' ? 'active' : ''} aria-current={tab === 'policy' ? 'page' : undefined} onClick={() => setTab('policy')}>정산 기준</button><button className={tab === 'history' ? 'active' : ''} aria-current={tab === 'history' ? 'page' : undefined} onClick={() => { setTab('history'); void load(); }}>변경 기록</button></nav>
      {tab === 'automation' && <OdaAutomation key={`${storeId}:${month}:${data.actor.id}`} storeId={storeId} month={month} canManage={['hq_master', 'store_owner'].includes(data.actor.role)} onApplied={() => void load()} />}
      ...
      {tab === 'expenses' && <OdaExpenses key={`${storeId}/${month}`} state={state} storeId={storeId} month={month} editable={editable} busy={Boolean(busy)}
        ...
        onBatch={(lineIds, changes, expectedVersion, options) => mutate('/expenses/batch', { lineIds, changes, ...options }, `비용 ${lineIds.length}건을 저장했습니다.`, expectedVersion)}
      ...
      {tab === 'policy' && policy && <div className="oda-policy-grid"><section className="oda-card"><div className="oda-card-head"><div><h2>이번 달 정산 기준</h2><p>저장된 기준을 다음 달에도 사용합니다. 변경된 항목만 수정하세요.</p></div><span className="oda-badge">{month}</span></div><OdaSalesChannels policy={policy} disabled={!editable || Boolean(busy)} onChange={setPolicy} />
...
function UploadBox({ storeId, month, disabled, busy, onImport, onReading, notify, requestedKind }: { requestedKind?: OdaSourceKind; storeId: string; month: string; disabled: boolean; busy: boolean; onImport: (files: PendingFile[], onSaved?: (file: PendingFile) => void) => Promise<boolean>; onReading: (reading: boolean) => void; notify: Props['notify'] }) {
    ...
    const settled = await Promise.allSettled(list.map(async (file) => {
      ...
      try {
        ...
        try {
          ...
          return { id, input: { ...input, expectedExpenseRulesVersion: preview.expenseRulesVersion }, preview };
  ...
  async function refreshFile(file: PendingFile) {
    if (disabled || checking) return;
    setReading(true); setReadProgress({ done: 0, total: 1 });
    try {
      const preview = await previewOdaImport(storeId, month, file.input);
      setFiles(current => current.map(item => item.id === file.id ? { ...file, input: { ...file.input, expectedExpenseRulesVersion: preview.expenseRulesVersion }, preview, error: undefined } : item));
    } catch (error) { setFiles(current => current.map(item => item.id === file.id ? { ...file, error: error instanceof Error ? error.message : '미리보기를 새로고침하지 못했습니다.' } : item)); }
  ...
  return <section className="oda-upload-card" aria-label="정산 자료 업로드">
    ...
    {files.length > 0 && <div className="oda-preview"><header><strong>반영 전 확인 · {files.length}개 파일</strong><button className="button button-ghost" aria-label="업로드 미리보기 닫기" disabled={busy || checking} onClick={() => setFiles([])}><X size={16} /></button></header>
      {files.map((file) => <div className="oda-preview-file" key={file.id}><Button variant="ghost" disabled={disabled || checking} onClick={() => void refreshFile(file)}>미리보기 새로고침</Button><div className="oda-preview-file-heading"><div><strong>{file.input.filename}</strong><p>{kinds[file.input.kind]} · {channelLabel(file.input.channel || '')} · <span className={file.error || file.preview?.errors.length ? 'oda-file-error' : 'oda-file-ready'}>{file.error || file.preview?.errors.length ? '수정 필요' : '반영 준비 완료'}</span></p></div><button className="button button-ghost" aria-label={`${file.input.filename} 업로드 목록에서 제거`} disabled={busy || checking} onClick={() => setFiles((current) => current.filter((item) => item !== file))}><X size={16} /></button></div>
...
function WorkbookMapping({ file, storeId, month, disabled, onUpdate, onWorking }: { file: PendingFile; storeId: string; month: string; disabled: boolean; onWorking: (working: boolean) => void; onUpdate: (file: PendingFile) => void }) {
  ...
  async function apply() {
    ...
    try { const preview = await previewOdaImport(storeId, month, input); onUpdate({ ...file, input: { ...input, expectedExpenseRulesVersion: preview.expenseRulesVersion }, preview, error: undefined }); }
