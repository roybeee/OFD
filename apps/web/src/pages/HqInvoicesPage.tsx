...
export function HqInvoicesPage({ data, notify, refresh }: { data: BootstrapData; notify: (message: string, tone?: 'success' | 'info' | 'warning') => void; refresh: () => void }) {
  ...
  async function approve(invoice: Invoice) {
    try {
      await mutateV2(`/invoices/${invoice.id}/approve`, { expectedVersion: invoice.version ?? 1 }, {
        idempotencyKey: newIdempotencyKey(),
      });
      setSelected(null);
      notify(`${invoice.storeName} 계산서를 승인했습니다.`, 'success');
      refresh();
  ...
  return (
    <main id="main-content" data-testid="hq-invoice-screen" className="page page-hq" tabIndex={-1}>
      <section className="page-heading hq-heading"><div><p className="eyebrow"><span /> HQ FINANCE</p><h1>정산·세금계산서</h1><p>재무 검토와 마스터 승인 상태를 운영 데이터로 확인합니다.</p></div><div className="heading-tools"><span className="deadline-badge"><CalendarCheck size={17} /> {deadline.label}</span></div></section>
      ...
      <section className="metrics-grid"><MetricCard label="총 청구액" value={formatMoney(totalGross)} detail={`${invoices.length}개 매장 · VAT 포함`} icon={<FileCheck2 size={21} />} /><MetricCard label="승인 필요" value={`${reviewed.length}건`} detail="현재 검토 상태 기준" icon={<UserCheck size={21} />} tone="red" /><MetricCard label="발행 대기" value={`${queued}건`} detail="서버 발급 상태 기준" icon={<Send size={21} />} tone="orange" /><MetricCard label="국세청 전송 완료" value={`${sent}건`} detail={`실패 ${invoices.filter((invoice) => invoice.status === 'failed').length}건`} icon={<CircleCheckBig size={21} />} tone="green" /></section>
