...
import { AlertCircle, ArrowDownToLine, ArrowRight, Check, ChevronRight, CircleDollarSign, Clock3, PackageCheck, Phone, ReceiptText, ShoppingBag, Truck } from '../components/icons';
...
export function StoreHomePage({ data, notify, onNavigate }: { data: BootstrapData; notify: Notify; onNavigate: (path: string) => void }) {
  ...
  return (
    <main id="main-content" data-testid="store-home-screen" className="page page-store page-owner" tabIndex={-1}>
      <section className="owner-greeting">
        <div>
      ...
      <div className="owner-grid">
        <section className="panel owner-panel" aria-labelledby="owner-money-title">
          <div className="owner-panel-head"><h2 id="owner-money-title">내야 할 금액</h2>{canSeeDocuments && <button type="button" className="owner-more" onClick={() => onNavigate('/store/documents')}>전체 보기 <ChevronRight size={15} aria-hidden="true" /></button>}</div>
          <p className="owner-amount">{formatMoney(unpaidTotal)}</p>
          <p className="owner-amount-note">{nextPayment ? `${nextPayment.title} · ${nextPayment.period}` : '지금 결제할 금액이 없어요.'}</p>
          <dl className="owner-facts">
            <div><dt>결제 조건</dt><dd>{data.store.paymentTerm}</dd></div>
            <div><dt>청구 방식</dt><dd>{data.store.billingPolicy}</dd></div>
          </dl>
        </section>

        <section className="panel owner-panel" aria-labelledby="owner-orders-title">
          <div className="owner-panel-head"><h2 id="owner-orders-title">발주 현황</h2>{canOrder && <button type="button" className="owner-more" onClick={() => onNavigate('/store/orders')}>전체 보기 <ChevronRight size={15} aria-hidden="true" /></button>}</div>
          <ul className="owner-counts">
            <li><span className="owner-count-icon" aria-hidden="true"><Clock3 size={18} /></span><strong>{waiting}건</strong><small>본사 확인 중</small></li>
            <li><span className="owner-count-icon" aria-hidden="true"><PackageCheck size={18} /></span><strong>{preparing ? '1건' : '0건'}</strong><small>상품 준비 중</small></li>
            <li><span className="owner-count-icon" aria-hidden="true"><Truck size={18} /></span><strong>{arriving ? '1건' : '0건'}</strong><small>배송 중</small></li>
          </ul>
          {ownOrders[0] && <p className="owner-latest">최근 발주 <strong>{ownOrders[0].code}</strong> · {readableDate(ownOrders[0].deliveryDate)} 입고 예정</p>}
        </section>
      </div>

      {canSeeDocuments && <section className="panel owner-panel" aria-labelledby="owner-docs-title">
        <div className="owner-panel-head"><h2 id="owner-docs-title">받은 증빙</h2><button type="button" className="owner-more" onClick={() => onNavigate('/store/documents')}>전체 보기 <ChevronRight size={15} aria-hidden="true" /></button></div>
